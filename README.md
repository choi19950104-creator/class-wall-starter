# 우리 반 담벼락

메모를 쓰면 올린 순서대로 붙는 담벼락입니다. 연수 실습의 공통 시작점입니다.

## 지금 되는 것

- 메모 쓰기 (내용 쓰고 엔터)
- Google 로그인·로그아웃
- 내가 쓴 메모 지우기 (× 버튼)
- 올린 순서대로 담벼락에 붙기
- Firestore에 메모 저장하기
- 교사가 Gemini로 모든 게시물에 AI 코멘트 남기기

## Gemini API 설정

AI 코멘트는 Vercel 서버리스 함수인 `/api/gemini`에서 만듭니다.
Vercel 프로젝트의 환경변수에 다음 두 값을 설정합니다.

- `GEMINI_API_KEY`: Google AI Studio에서 발급한 Gemini API 키
- `FIREBASE_API_KEY`: Firebase 웹 앱의 API 키

환경변수를 설정한 뒤 Vercel을 다시 배포해야 합니다. Live Server는 `api/` 함수를
실행하지 않으므로 AI 기능을 로컬에서 확인할 때는 프로젝트 폴더에서 `npx vercel dev`를 사용합니다.

## 교사·학생 역할 설정

처음 Google로 로그인한 사용자는 Firestore의 `users/{uid}` 문서에
`role: "student"`로 자동 등록됩니다.

교사로 지정하려면 Firebase 콘솔의 Firestore에서 해당 사용자 문서를 열고
`role` 값을 `teacher`로 바꿉니다. 교사는 모든 메모를 관리할 수 있고,
학생은 자신의 메모만 만들고 수정하거나 지울 수 있습니다.

## 실행 방법

`index.html`을 **더블클릭하면 화면이 비어 있습니다.** 다음 방법으로 여세요.

Antigravity(또는 VS Code)에서 `Live Server` 확장을 설치하고,
`index.html`에 마우스 오른쪽 버튼 → `Open with Live Server`.

`http://127.0.0.1:5500` 같은 주소로 열리면 성공입니다.

> 왜 더블클릭하면 안 되나요?
> `app.js`를 모듈 방식으로 불러오는데, 브라우저는 보안 때문에
> 주소가 `file://`로 시작하면 모듈을 막습니다.
> Firebase를 붙이려면 이 방식이어야 해서, 처음부터 이렇게 해 두었습니다.

## 파일

| 파일 | 하는 일 |
|---|---|
| `index.html` | 화면 뼈대와 스타일 |
| `app.js` | 메모 데이터, 읽기·쓰기·지우기, 화면 그리기 |
| `api/gemini.js` | 교사 권한을 확인하고 Gemini를 호출하는 Vercel 서버 코드 |

## 이 프로젝트의 규칙

Firebase는 **무료 요금제(Spark)** 로 진행합니다.

그래서 **Firebase Functions는 쓰지 않습니다.** 유료 요금제(Blaze)라야 동작하기 때문입니다.
서버가 필요한 일(Gemini 호출)은 **Vercel 서버리스 함수**로 만듭니다. `api/` 폴더가 그 자리입니다.

> AI에게 Gemini 연결을 시키면 대개 Firebase Functions 코드를 줍니다.
> 프롬프트에 **"Vercel 서버리스 함수로 만들어 줘"** 라고 꼭 적으세요.

자세한 규칙은 `AGENTS.md`에 있습니다. AI 코딩 도구가 이 파일을 읽습니다.
